...
from fastapi import FastAPI, HTTPException #Xây dựng API và xác thực input
from pydantic import BaseModel, ValidationError, Field
import logging #Debug
import random #dùng để chọn nước đi ngẫu nhiên
...
class Othello:
    def __init__(self, board=None, current_player=-1):
        ...
        self.last_passed_by_ai = False  # Theo dõi xem lần pass cuối cùng có phải do AI không
    ...
    def get_winner(self):
        ...
        if surrender:
            return result  # Người chơi (Đen) thắng
        if white > black:
            return 1
        elif black > white:
            return -1
...
class MCTS:
    ...
    def search(self, game):
        ...
        # Chọn ngẫu nhiên 1 nước đi từ các nước đi có số lần thăm ít nhất
        min_visits = min(self.N.get((root_state, tuple(move)), 0) for move in valid_moves)
        worst_moves = [move for move in valid_moves if self.N.get((root_state, tuple(move)), 0) == min_visits]
        worst_move = random.choice(worst_moves)
        return worst_move
    #Hàm mô phỏng
...
# Tải mô hình
try:
    model = keras.models.load_model('othello_model_easy.keras')
...
async def play_move(request: MoveRequest):
    try:
        ...
        if player_move:
            ...
            logger.info(f"Player (Black) move at {player_move}, flipped: {flipped}")
        else:
        ...
        if ai_move:
            ...
            logger.info(f"AI (White) move at {ai_move}, flipped: {flipped}")
            ...
            game.pass_turn()
            #Kiểm tra trò chơi sau khi pass
            surrender, _ = game.check_ai_surrender()
            if game.is_game_over() or surrender:
                return MoveResponse(
                    aiPositionX=None,
...
async def reset_game():
    global game
    game = Othello(current_player=-1)
    logger.info("Game board reset, starting with Black")
